...
class QueryExpansion():
    def __init__(self, args):
        ...
        self.qe = args.qe
        self.topk = args.topk
        self.stack_num = args.qe_stacknum
        self.qf = args.qe_filter
    ...
    def __call__(self, args, topk_list, querydb, indexdb, query_dataset, index_dataset):
        if self.qe==0:
            topk_newlist = self.average_query_expansion(args, topk_list, querydb, indexdb, query_dataset, index_dataset)
        return topk_newlist


    def average_query_expansion(self, args, topk_list, querydb, indexdb, query_dataset, index_dataset):
        ...
        posk_list_all = []
        num_inlier_all = []
        if self.stack_num>=self.topk:
            iter_desc = "Current retrieval topk is {}, qe stack K is {} recommend increase topk!".format(self.topk, self.stack_num)
        else:
            iter_desc = "Verify topk!"
        ...
        for qi, index_i_list in enumerate(tqdm.tqdm(topk_list, desc=iter_desc)):
            ...
            for ni, neigh in enumerate(index_i_list):
                ...
                num_inlier,_,_,_ = self.local_match(query_kps, query_desc, query_score, query_etc, \
                                              index_kps, index_desc, index_score, index_etc)
                if num_inlier>self.qf:
                    num_inlier_sub.append(num_inlier)
                    posk_list_sub.append(neigh)
                    cnt+=1
                if cnt==self.stack_num:
                    break
            
            for i in range(cnt, self.stack_num):
                num_inlier_sub.append(-1)
                posk_list_sub.append(-1)
        ...
        query_avgFeats = []
        for quei, index_i_list in enumerate(tqdm.tqdm(posk_list_all, desc="Query expansion!")):
            valid_mask = (index_i_list!=-1)
            if valid_mask.sum()==0:
                query_avgFeats.append(query_feats[quei])
                continue
            curr_queryF = np.expand_dims(query_feats[quei], axis=0)
            curr_indexF = index_feats[index_i_list[valid_mask]]
            curr_setF = np.concatenate((curr_queryF, curr_indexF), axis=0)
            curr_avgF = np.mean(curr_setF, axis=0)
            query_avgFeats.append(curr_avgF)
        query_avgFeats = np.asarray(query_avgFeats)
        ...
        topk_newlist = knn.predict(np.ascontiguousarray(query_avgFeats, dtype=np.float32), self.topk)
        knn.delete()
        del knn
        return topk_newlist
    ...
    def local_match(self, query_kps, query_desc, query_score, query_etc, \
        ...
        return num_inlier, pts_query, None, conf