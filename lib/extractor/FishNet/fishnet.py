...
class Fish(nn.Module):
    def __init__(self, block, num_cls=1000, num_down_sample=5, num_up_sample=3, trans_map=(2, 1, 0, 6, 5, 4),
                 network_planes=None, num_res_blks=None, num_trans_blks=None, layer=9):
        super(Fish, self).__init__()
        self.block = block
        self.trans_map = trans_map
        self.upsample = Interp(scale_factor=2)
    ...
    def forward(self, x):
        ...
        feat_list = self._fish_forward(all_feat)
        return feat_list[self.layer]
