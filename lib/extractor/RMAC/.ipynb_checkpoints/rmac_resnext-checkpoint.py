...
def resnext18_rmac(backbone=ResNext_RMAC, **kwargs):
    kwargs.pop('scales', None)
    return backbone(BasicBlock, [2, 2, 2, 2], 'resnext18', **kwargs)

def resnext50_rmac(backbone=ResNext_RMAC, **kwargs):
    kwargs.pop('scales', None)
    return backbone(LambdaBase, [3, 4, 6, 3], 'resnext50', **kwargs)

def resnext101_rmac(backbone=ResNext_RMAC, **kwargs):
    kwargs.pop('scales', None)
    return backbone(LambdaBase, [3, 4, 23, 3], 'resnext101', **kwargs)

def resnext152_rmac(backbone=ResNext_RMAC, **kwargs):
    kwargs.pop('scales', None)
    return backbone(LambdaBase, [3, 8, 36, 3], 'resnext152', **kwargs)
