...
class Ensemble3(nn.Module):
    def __init__(self, network1, network2, network3, is_backbone1=False, is_backbone2=False, is_backbone3=False, dim=[4096, 4096, 4096]):
    ...
    def postprocessing(self, index_feat, test_feat):
        ...
        if self.is_backbone1 is False:
            if self.size1>self.dim[0]:
                pca = pp.PCAwhitening(pca_dim=self.dim[0], pca_whitening=True)
                index_feat1 = pca.fit_transform(index_feat1)      
                test_feat1 = pca.transform(test_feat1)
        if self.is_backbone2 is False:
            if self.size2>self.dim[1]:
                pca = pp.PCAwhitening(pca_dim=self.dim[1], pca_whitening=True)
                index_feat2 = pca.fit_transform(index_feat2)      
                test_feat2 = pca.transform(test_feat2)
        if self.is_backbone3 is False:
            if self.size3>self.dim[2]:
                pca = pp.PCAwhitening(pca_dim=self.dim[2], pca_whitening=True)
...
class Ensemble2(nn.Module):
    def __init__(self, network1, network2, is_backbone1=False, is_backbone2=False, dim=[4096, 4096]):
    ...
    def forward(self, x):
        ...
        self.size1, self.size2 = out1.shape[1], out2.shape[1]
    ...
    def postprocessing(self, index_feat, test_feat):
        ...
        index_feat1 = index_feat[:, :self.size1]
        index_feat2 = index_feat[:, self.size1:]

        test_feat1 = test_feat[:, :self.size1]
        test_feat2 = test_feat[:, self.size1:]
        ...
        index_feat = np.concatenate((index_feat1, index_feat2), axis=1)
        test_feat = np.concatenate((test_feat1, test_feat2), axis=1)
